...
const input = JSON.parse(require('fs').readFileSync('./day2.input.json').toString());

const blueInBag = 14;
const redInBag = 12;
const greenInBag = 13;

const games = input.reduce((all, game) => {
    const {gameId, sets} = game.match(/^Game (?<gameId>\d+):(?<sets>.*)$/)?.groups ?? {};

    all[gameId] = {blue: 0, green: 0, red: 0};

    // For each Set
    sets.split(';').forEach((set) => {
        // For Each Draw
        set.split(',').forEach((draw) => {
            // Get colour and Total drawn
            const {total, colour} = draw.match(/ ?(?<total>\d+) (?<colour>blue|red|green)/)?.groups ?? {};

            const totalInt = parseInt(total, 10);
            // Store max drawn for this colour in this game
            if (all[gameId][colour] < totalInt) {
                all[gameId][colour] = totalInt;
            }
        });
    });
    return all;
...
console.log('Part 1 = %d', Object.entries(games).reduce((total, [id, {blue, red, green}]) => {
    if (blue > blueInBag) return total;
    if (red > redInBag) return total;
    if (green > greenInBag) return total;
...
//     green, and 2 blue cubes in the bag.
//
...
console.log('Part 2 = %d', Object.entries(games).reduce((total, [id, {blue, red, green}]) => {
