...
const input = JSON.parse(require('fs').readFileSync('./day7.input.json').toString());

const data = input.map((line) => {
    return line.split(' ');
});

const normalizeCardValue = (char, jokerChar) => {
    switch (char) {
        case jokerChar: return 1
        case 'A': return 14
        case 'K': return 13
        case 'Q': return 12
        case 'J': return 11
...
const handSorter = ([[hand1, hand1Value]], [[hand2, hand2Value]], joker) => {
    ...
    if (hand2Value > hand1Value) return -1;

    for (let i = 0; i < hand1.length; i++) {
        const card1Value = normalizeCardValue(hand1.charAt(i), joker);
        const card2Value = normalizeCardValue(hand2.charAt(i), joker);

        if (card1Value > card2Value) return 1;
        if (card2Value > card1Value) return -1;
    }

    return 0;
}

console.log('Part 1 = %d', data.map(([hand, bid]) => [[hand, handValue(hand)], bid])
    .sort(handSorter)
    .reduce((total, [[hand, value], bid], index) => total += bid * (index + 1), 0));
...
console.log('Part 2 = %d', data.map(([hand, bid]) => [[hand, handValue(hand, 'J')], bid])
    .sort((hand1, hand2) => handSorter(hand1, hand2, 'J'))
    .reduce((total, [[hand, value], bid], index) => total += bid * (index + 1), 0));
