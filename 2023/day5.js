...
//     '56 93 4',
// ];
const input = JSON.parse(require('fs').readFileSync('./day5.input.json').toString());

let mapName = '';
const data = input.reduce((dataMap, line) => {
    if (line.match(/^seeds:[\d ]+$/)) {
        dataMap.seeds = line.substr(7).split(' ').map((item) => parseInt(item, 10));
        return dataMap;
    }

    if (line.match(/^[a-zA-Z:-]/)) {
        mapName = line.replace(/-to-.*:/, '');
        return dataMap;
    }

    const [destinationStart, sourceStart, length] = line.split(' ').map((item) => parseInt(item, 10));

    dataMap[mapName].push({
        sourceStart,
    ...
    return dataMap;
}, {seeds: [], seed: [], soil: [], fertilizer: [], water: [], light: [], temperature: [], humidity: []});

// Find the first map that contains our sourceId. then return the destination id using the offset between the sourceId and the maps source start
// and get the destinationId at the same offset from the maps destination starthh s
const getDestinationByRanges = (sourceName, [start, end], allowDefault = true) => {
    const map = Object.values(data[sourceName])
        .filter(({sourceStart, sourceEnd}) => {
            if (start >= sourceStart && start <= sourceEnd) return true;

            return false;
        }).reduce((ranges, {sourceStart, sourceEnd, destinationStart, destinationEnd}) => {
            // Reduce search space down to input length
            const deltaS = start - sourceStart;
            const deltaE = end - sourceEnd;
            ranges.push([destinationStart + deltaS, destinationEnd + deltaE]);

            // If the end of the source range extends beyond out range, then calc for that bit
            const diff = end - sourceEnd;
            if (diff > 0) {
                ranges.push(...getDestinationByRanges(sourceName, [sourceEnd + 1, end], false));
            }
    ...
    if (map.length === 0) {
        if (allowDefault) return [[start, end]];
        return [];
    }

    const uniqRanges = Object.entries(map.reduce((unique, [start, end]) => {
        unique[start] = end;
        return unique;
...
const getLocationByRanges = (seedRange) => {
    const soilRanges = getDestinationByRanges('seed', seedRange);
    const fertilizerRanges = soilRanges.map((range) => getDestinationByRanges('soil', range)).reduce((a,b) => a.concat(b));
    const waterRanges = fertilizerRanges.map((range) => getDestinationByRanges('fertilizer', range)).reduce((a,b) => a.concat(b));
    const lightRanges = waterRanges.map((range) => getDestinationByRanges('water', range)).reduce((a,b) => a.concat(b));
    const temperatureRanges = lightRanges.map((range) => getDestinationByRanges('light', range)).reduce((a,b) => a.concat(b));
    const humidityRanges = temperatureRanges.map((range) => getDestinationByRanges('temperature', range)).reduce((a,b) => a.concat(b));
    const locationRanges = humidityRanges.map((range) => getDestinationByRanges('humidity', range)).reduce((a,b) => a.concat(b));
    return locationRanges.reduce((min, [start]) => min && min < start ? min: start, Number.MAX_SAFE_INTEGER);
}

console.log('Part 1 = %o', data.seeds.reduce((minLocation, seedId) => {
    const locationId = getLocationByRanges([seedId, seedId]);

    if (!minLocation) return locationId;

    return locationId < minLocation ? locationId : minLocation;
}, 0));

console.log('Part 2 = %o', data.seeds
    .reduce((tuples, item, index, seeds) => {
        return index % 2 === 0 ? [...tuples, seeds.slice(index, index + 2)] : tuples;
    }, [])
    .reduce((minLocation, [start, length]) => {
        const locationId = getLocationByRanges([start, start + length - 1]);
        if (!minLocation) minLocation = locationId;
        if (minLocation > locationId) minLocation = locationId;
