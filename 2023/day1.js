...
// Part 1
const input = JSON.parse(require('fs').readFileSync('./day1.input.json').toString());

console.log('Part 1 = %d', input.map((value) => {
    const numerics = value.replace(/['a-zA-Z']/g, '');
    const result = parseInt(`${numerics.slice(0, 1)}${numerics.slice(numerics.length - 1, numerics.length)}`, 10);
...
console.log('Part 2 = %d', input.map((value) => {
    const numerics = convertNumberWords(value).replace(/[a-zA-Z]/g, '');
    const result = parseInt(`${numerics.slice(0, 1)}${numerics.slice(numerics.length - 1, numerics.length)}`, 10);
    console.log('%s -> %s = %d', value, numerics, result);
