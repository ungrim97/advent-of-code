...
const input = JSON.parse(require('fs').readFileSync('./day4.input.json').toString());

const cards = input.reduce((all, card) => {
    const [matches, cardId, winningNumbers, cardNumbers] = card.match(/^Card[^\d]+(\d+): ([\d ]+) \| ([\d ]+)$/) ?? ['', '', ''];
...
console.log('Part 1 = %d', Object.entries(cards).reduce((total, [id, {winningNumbers, cardNumbers}]) => {
    const matchedNumbers = winningNumbers.filter((number) => cardNumbers.includes(number));
    const value = matchedNumbers.length ? (2 ** (matchedNumbers.length - 1)) : 0;
...
//     numbers, so you win one copy each of cards 3 and 4.
//
...
const getTotalPlays = () => {
    const {plays} = Object.entries(cards).reduce(({plays}, [id, {winningNumbers, cardNumbers}]) => {
        const matchedNumbers = winningNumbers.filter((number) => cardNumbers.includes(number));

        plays[id] = (plays[id] ?? 0) + 1;

        for (let i = 1; i <= matchedNumbers.length; i++) {
            const newId = parseInt(id, 10) + i;
            plays[newId] = (plays[newId] ?? 0) + plays[id];
        }
    ...
    return Object.entries(plays).reduce((total, [id, playCount]) => total += parseInt(playCount, 10), 0);
}
console.log('Part 2 = %d', getTotalPlays());
