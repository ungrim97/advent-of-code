...
//     of travel time, the boat will have gone 12 millimeters. Hold the button
//     for 5 milliseconds, causing the boat to travel a total of 10
//     millimeters.
//
...
//     'Distance:  9  40  200',
// ];
const input = JSON.parse(require('fs').readFileSync('./day6.input.js').toString());

const data = input.reduce((races, line, index) => {
    const [_, ...raceData] = line.replaceAll(/[a-zA-Z: ]+/g, ' ').split(' ');

    raceData.map((datum, raceIndex) => {
        if (!races[raceIndex]) races[raceIndex] = [];
        races[raceIndex][index] = Number(datum);
    })
...
console.log('Part 1 = %d', data.reduce((total, [time, maxDistance]) => {
    ...
    if (solutions === 0) return total;
    if (total === 0) return solutions;
