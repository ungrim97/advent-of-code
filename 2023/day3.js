...
const input = JSON.parse(require('fs').readFileSync('./day3.input.json').toString());

const data = input.map((line, index) => {
    const groups = line.match(/(\d+|\.+|[^\.\d]+)/g)?.reduce((all, value, itemIndex) => {
        const [previous] = all.slice(-1);
        const start = (previous?.end ?? -1) + 1;
    ...
    return groups.filter((item) => item.value.match(/[^\.]+/));
});

const findSymbol = (currentItem, searchItem) => {
    if (searchItem.value.match(/^\d/)) return false;
    const start = searchItem.start -1;
    const end = searchItem.end + 1;

    if ((currentItem.start >= start) && (currentItem.start <= end)) return true;

    if ((currentItem.end >= start) && (currentItem.end <= end )) return true;

    return false;
}

console.log('Part 1 = %d', data.reduce((total, line, lineIndex) => {
    const previousLine = data[lineIndex - 1];
    const nextLine = data[lineIndex + 1];

    line.forEach((currentItem ) => {
        if (currentItem.value.match(/^[^\d]/)) return;

        if (!!previousLine?.find((item) => findSymbol(currentItem, item))) {
            return total += parseInt(currentItem.value, 10);
        }

        if (!!line?.find((item) => findSymbol(currentItem, item))) {
            return total += parseInt(currentItem.value, 10);
        }

        if (!!nextLine?.find((item) => findSymbol(currentItem, item))) {
            return total += parseInt(currentItem.value, 10);
        }
    });
...
const findNumber = (currentItem, searchItem) => {
    if (searchItem.value.match(/^[^\d]/)) return false;
    const start = searchItem.start -1;
    const end = searchItem.end + 1;
...
console.log('Part 2 = %d', data.reduce((total, line, lineIndex) => {
    const previousLine = data[lineIndex - 1] ?? [];
    const nextLine = data[lineIndex + 1] ?? [];

    line.forEach((currentItem) => {
        if (currentItem.value.match(/^[^\*]/)) return;

        const availableNumbers = []
            .concat(previousLine.filter((item) => findNumber(currentItem, item)))
            .concat(line.filter((item) => findNumber(currentItem, item)))
            .concat(nextLine.filter((item) => findNumber(currentItem, item)));


        if (availableNumbers.length !== 2) return total;

        const [{value: match1}, {value: match2}] = availableNumbers;
        const ratio = parseInt(match1, 10) * parseInt(match2, 10);
